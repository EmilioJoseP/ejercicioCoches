...
public interface VehiculoRepository extends JpaRepository<Vehiculo, Long> {
    ...
    /*@Query(value = "SELECT v.matricula_vehiculo, v.id_vehiculo, v.id_modelo, v.pintura, v.fecha_matriculacion, v.combustible, mo.nombre_modelo, ma.nombre_marca " +
            "FROM vehiculos v " +
            "LEFT JOIN modelos mo ON v.id_modelo = mo.id_modelo " +
            "LEFT JOIN marcas ma ON mo.id_marca = ma.id_marca " +
            "WHERE v.id_vehiculo = :idVehiculo", nativeQuery = true)
    Vehiculo findVehiculo(int idVehiculo);*/
}
